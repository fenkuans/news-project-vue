...
export default {
    ...
    methods: {
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                    ...
                    if (this.verifyForm()){
                        ...
                        this.$http.post('/news/update_news',this.ruleForm)
                        .then((req)=>{
                            console.log(req);
        ...
        initViewDataEvent() {
            ...
            //已上串图片的回现
            this.getMiniImgData(this.ruleForm.minImgUrl);
        },
        getMiniImgData(minImgUrl) {
            
            let x = minImgUrl.indexOf('/');
            for (let i=0;i<2;i++){
                x=minImgUrl.indexOf('/',x+1);
            }

            let outUrl = minImgUrl.substring(x+1);
            this.fileList = [
                {
                    name: outUrl,
                    url: this.$host+minImgUrl
                }
            ]
        }
    },
    ...
    mounted(){
        ...
        this.$http.get('/news/news_items',{params:{
            newsId: id
        }})
        .then((req)=>{
            if (req.status === 200){
